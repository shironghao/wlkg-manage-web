...
<!--          <v-btn icon v-else @click="saleableGoods(props.item)">上架</v-btn>-->

          <v-btn icon  @click="saleableGoods(props.item)">{{props.item.saleable ? '下架' : '上架'}} </v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        ...
        this.$message.confirm('此操作将删除该商品, 是否继续?').then(() => {
          // 发起删除请求
          this.$http.delete("/item/goods/delete?id=" + item.id,)
            .then(() => {
              // 删除成功，重新加载数据
      ...
      saleableGoods(item) {
        this.$message.confirm(item.saleable?'此操作将该商品下架, 是否继续?':'此操作将该商品上架, 是否继续?').then(() => {
          // 发起下架请求
          this.$http.put("/item/goods/saleable?id=" + item.id,)
            .then(() => {
              // 删除成功，重新加载数据
              this.$message.success(item.saleable?"下架成功！":"上架成功！");
              this.getDataFromServer();
            })
        })
      },
